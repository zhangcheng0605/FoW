...
function prSpotRect(l, t, w, h) {
  ...
  /* keep the caption out of the glow — hop to the top when the subject sits low */
  const cap = $("#prCap");
  if (cap) cap.classList.toggle("up", t + h > innerHeight - 235 && t > 300);
}
function prSpotlight(target, pad) {
  ...
  if (!target) {
    ...
    const cap = $("#prCap"); if (cap) cap.classList.remove("up");
    return;
  }
  const p = pad == null ? 10 : pad;
  /* live-tracked: re-measured on a timer so scroll drift and reflows never strand the glow */
  PRESENT.spotFn = () => {
    if (!document.contains(target)) return;
    const r = target.getBoundingClientRect();
    prSpotRect(r.left - p, r.top - p, r.width + p * 2, r.height + p * 2);
  };
  PRESENT.spotFn();
}
async function prScrollTo(target) {
  ...
  /* wait until the smooth scroll genuinely settles — rects are read right after,
     and the settle time is physical, so it is deliberately not speed-scaled */
  let last = Infinity, still = 0, guard = 50;
  while (guard-- > 0) {
    if (PRESENT.abort) throw { aborted: true };
    if (PRESENT.skip) return;
    await new Promise(r => setTimeout(r, 90));
    const t = target.getBoundingClientRect().top;
    if (Math.abs(t - last) < 0.75) { if (++still >= 3) break; }
    else still = 0;
    last = t;
  }
  await psleep(150);
  ...
  add(async () => {
    ...
    if (kpis.length) {
      ...
      PRESENT.spotFn = () => {
        const ks = $$(".card.kpi");
        if (!ks.length) return;
        const all = ks.map(k => k.getBoundingClientRect());
        ...
        prSpotRect(L - 10, T - 10, R - L + 20, B - T + 20);
      };
      PRESENT.spotFn();
    }
    showBeat("kpis");
...
async function startPresent(personaId) {
  ...
  PRESENT.allowCrew = false;
  PRESENT.spotFn = null;
  clearInterval(PRESENT.track);
  PRESENT.track = setInterval(() => {
    try { if (PRESENT.on && !PRESENT.paused && PRESENT.spotFn) PRESENT.spotFn(); } catch (_) { }
  }, 240);
